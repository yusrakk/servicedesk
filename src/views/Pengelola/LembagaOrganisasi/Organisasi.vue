...
const fetchData = () => {
  ...
  axios.get('/api/organisasi', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
      dataOrganisasi.value = res.data.map(item => ({
...
const filteredItems = computed(() => {
  ...
  return dataOrganisasi.value.filter(i =>
    i.nama_PerangkatDaerah.toLowerCase().includes(q) ||
    i.induk_PerangkatDaerah.toLowerCase().includes(q) ||
...
function openDelete(item) {
  idToDelete.value = item.id_organisasi
  namaToDelete.value = item.nama_PerangkatDaerah
  showModal.value = true
  startCountdown()
}
function cancelDelete() { showModal.value = false; idToDelete.value = null; clearInterval(timer); isCounting.value = false }
function confirmDelete() {
  const token = localStorage.getItem('Token')
  axios.delete(`/api/organisasi/${idToDelete.value}`, { headers: { Authorization: 'Bearer ' + token } })
    .then(() => { fetchData(); showModal.value = false })
    .catch(e => alert(e.response?.data?.message || 'Gagal menghapus organisasi.'))
}

function editOrganisasi(item) {
  router.push({ path: '/ubahLembaga', query: { id_organisasi: item.id_organisasi, id_induk_organisasi: item.id_induk_organisasi, nama_PerangkatDaerah: item.nama_PerangkatDaerah, indukOrganisasi: item.induk_PerangkatDaerah, email: item.email, status: item.status } })
}
function lihatOrganisasi(item) {
  router.push({ path: '/ubahLembaga', query: { id_organisasi: item.id_organisasi, id_induk_organisasi: item.id_induk_organisasi, nama_PerangkatDaerah: item.nama_PerangkatDaerah, indukOrganisasi: item.induk_PerangkatDaerah, email: item.email, status: item.status, viewOnly: 'true' } })
}
</script>
  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <h1 class="page-header__title">Lembaga & Organisasi</h1>
        <p class="page-header__sub">Kelola data perangkat daerah dan induk organisasi</p>
      </div>
      <button class="btn-tambah" @click="router.push('/tambahLembaga')">
    ...
    <div class="body">
      <div class="toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" placeholder="Cari nama, induk, email, status..." class="search-input"/>
        ...
        <span class="result-count">{{ filteredItems.length }} organisasi</span>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="filteredItems.length === 0" class="state-center">
          <svg width="44" height="44" viewBox="0 0 44 44" fill="none"><rect x="6" y="8" width="32" height="28" rx="3" stroke="currentColor" stroke-width="1.5"/><path d="M14 18h16M14 24h10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
          <p>{{ search ? 'Tidak ada hasil pencarian' : 'Belum ada data organisasi' }}</p>
        </div>
              ...
              <tr v-for="(item, i) in paginatedItems" :key="i" class="table-row">
                <td>
                  <div class="org-name">
                    ...
                    <span class="org-text">{{ item.nama_PerangkatDaerah }}</span>
                  </div>
                </td>
                <td><span class="meta-text">{{ item.induk_PerangkatDaerah }}</span></td>
                <td><span class="email-text">{{ item.email }}</span></td>
                <td>
                  <span class="badge" :style="{ color: statusConfig[item.status]?.color||'#374151', background: statusConfig[item.status]?.bg||'#f3f4f6', borderColor: statusConfig[item.status]?.border||'transparent' }">
                    <span class="sdot" :style="{ background: statusConfig[item.status]?.color }"></span>
                    {{ item.status }}
                  </span>
                </td>
                <td>
                  <div class="action-group">
                    <button class="act-btn act-btn--detail" @click="lihatOrganisasi(item)" title="Detail">
                    ...
                    <button class="act-btn act-btn--edit" @click="editOrganisasi(item)" title="Ubah">
    ...
    <Teleport to="body">
      <div v-if="showModal" class="modal-overlay" @click.self="cancelDelete">
        <div class="modal">
          ...
          <h3 class="modal__title">Hapus Organisasi</h3>
          <p class="modal__desc">Yakin ingin menghapus <strong>{{ namaToDelete }}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
          <div v-if="isCounting" class="modal__countdown">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="6" stroke="currentColor" stroke-width="1.3"/><path d="M7 4v3.5l2 1.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
...
.org-name{display:flex;align-items:center;gap:.625rem;}
.org-icon{width:28px;height:28px;border-radius:7px;background:var(--cfo);color:var(--cm);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.org-text{font-size:.875rem;font-weight:600;color:var(--ci);}
.meta-text{font-size:.8rem;color:var(--cs);}
.email-text{font-size:.8rem;color:var(--cs);font-family:monospace;}
...
.mbutton--red:hover:not(:disabled){background:#dc2626;}
