...
const statusConfig = {
  ...
  'Diproses':      { color: '#d97706', bg: '#fef3c7' },
...
onMounted(() => {
  ...
  axios.get('/api/pelayanan', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
...
function lihatDetail(item) {
  router.push({ name: 'Detail-Pelayanan', query: { layanan: item.noTiket } })
}
  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <h1 class="page-header__title">Data Pelayanan</h1>
        <p class="page-header__sub">Kelola seluruh permintaan pelayanan yang masuk</p>
      </div>
    ...
    <div class="body">
      <div class="toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" placeholder="Cari tiket, perihal, perangkat daerah, status..." class="search-input"/>
        ...
        <span class="result-count">{{ filteredItems.length }} tiket</span>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="filteredItems.length === 0" class="state-center">
          ...
          <p>{{ search ? 'Tidak ada hasil pencarian' : 'Belum ada data pelayanan' }}</p>
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, i) in paginatedItems" :key="i" class="table-row" @click="lihatDetail(item)">
                ...
                <td><span class="meta-text">{{ item.organisasi }}</span></td>
                <td><span class="meta-text">{{ formatDate(item.tanggal) }}</span></td>
                <td>
                  <span class="badge" :style="{ color: urgensiConfig[item.prioritas]?.color||'#374151', background: urgensiConfig[item.prioritas]?.bg||'#f3f4f6' }">
                    {{ item.prioritas }}
                  </span>
                </td>
                <td>
...
.page-header{background:var(--cf);padding:2rem 2rem 2.5rem;margin:0 0 1.5rem;position:relative;overflow:hidden;display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;border-radius:20px;}
...
.search-box{flex:1;display:flex;align-items:center;gap:.625rem;background:var(--cw);border:1.5px solid rgba(168,200,180,.3);border-radius:12px;padding:.625rem .875rem;box-shadow:var(--sh);transition:border-color .15s,box-shadow .15s;}
...
.detail-btn{display:inline-flex;align-items:center;gap:.375rem;padding:.375rem .875rem;background:var(--cfo);border:1px solid rgba(46,184,106,.2);border-radius:8px;font-family:var(--fn);font-size:.75rem;font-weight:700;color:var(--ce);cursor:pointer;transition:all .15s;}
