...
const fetchData = () => {
  ...
  axios.get('/api/urgensi', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
      referensiUrgensi.value = res.data.map(i => ({
        id: i.ID_Urgensi, nama: i.Nama_Urgensi, tglPembuatan: i.created_at || '-'
      }))
    })
    .catch(console.error)
...
const filteredItems  = computed(() => {
  ...
  return referensiUrgensi.value.filter(i => i.nama.toLowerCase().includes(q))
})
...
function confirmDelete()  {
  const token = localStorage.getItem('Token')
  axios.delete(`/api/urgensi/${toDelete.value.id}`, { headers: { Authorization: 'Bearer ' + token } })
    .then(() => { fetchData(); showModal.value = false })
    .catch(e => alert(e.response?.data?.message || 'Gagal menghapus urgensi.'))
}
function editUrgensi(item) {
  router.push({ path: '/ubahUrgensi', query: { urgensiId: item.id, nama_urgensi: item.nama } })
}
</script>
  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <h1 class="page-header__title">Referensi Urgensi</h1>
        <p class="page-header__sub">Kelola tingkat urgensi yang digunakan dalam sistem pelayanan</p>
      </div>
      <button class="btn-tambah" @click="router.push('/tambahUrgensi')">
    ...
    <div class="body">
      <div class="toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" placeholder="Cari nama urgensi..." class="search-input"/>
        ...
        <span class="result-count">{{ filteredItems.length }} urgensi</span>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="filteredItems.length === 0" class="state-center">
          ...
          <p>{{ search ? 'Tidak ada hasil pencarian' : 'Belum ada data urgensi' }}</p>
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, i) in paginatedItems" :key="i" class="table-row">
                ...
                <td>
                  <span class="urgensi-badge"
                    :style="{ color: urgensiConfig[item.nama]?.color||'#374151', background: urgensiConfig[item.nama]?.bg||'#f3f4f6' }">
                    <span class="udot" :style="{ background: urgensiConfig[item.nama]?.color||'#374151' }"></span>
                    {{ item.nama }}
                  </span>
                </td>
                ...
                <td>
                  <div class="action-group">
                    <button class="act-btn act-btn--edit" @click="editUrgensi(item)" title="Ubah">
    ...
    <Teleport to="body">
      <div v-if="showModal" class="modal-overlay" @click.self="cancelDelete">
        <div class="modal">
          ...
          <h3 class="modal__title">Hapus Urgensi</h3>
          <p class="modal__desc">Yakin ingin menghapus urgensi <strong>{{ toDelete.nama }}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
...
.urgensi-badge{display:inline-flex;align-items:center;gap:.4rem;padding:.3rem .75rem;border-radius:99px;font-size:.78rem;font-weight:700;}
