...
const urgensiIdToDelete = ref("")
function Delete(item) {
  urgensiToDelete.value = item.nama
  urgensiIdToDelete.value = item.id
...
function cancelDelete() {
  ...
  urgensiIdToDelete.value = null
}

function confirmDelete() {
  const token = localStorage.getItem('Token');
  axios.delete(`/api/urgensi/${urgensiIdToDelete.value}`, {
  headers: {
      Authorization: 'Bearer ' + token
    }
  })
  ...
  .catch(error => {
    console.error(error);
    alert(error.response?.data?.message || 'Terjadi kesalahan saat menghapus urgensi.');
  });
}
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th @click="sortKey = 'nama'">Nama Urgensi</th>
