...
function handleSubmit() {
  if (!namaUrgensi.value.trim()) { alert('Nama urgensi wajib diisi'); return }
  isSubmitting.value = true
  const token = localStorage.getItem('Token')
  axios.post('/api/urgensi', { Nama_Urgensi: namaUrgensi.value }, { headers: { Authorization: 'Bearer ' + token } })
    .then(() => router.push('/referensi/urgensi'))
    .catch(e => alert(e.response?.data?.message || 'Gagal menambah urgensi.'))
...
<template>
  <div class="form-page">
    <div class="page-header">
      <button class="back-btn" @click="router.push('/referensi/urgensi')">
      ...
      <div>
        <span class="page-header__label">Referensi · Urgensi</span>
        <h1 class="page-header__title">Tambah Urgensi</h1>
        <p class="page-header__sub">Tambahkan tingkat urgensi baru ke dalam sistem</p>
      </div>
      <div class="orb"></div>
    </div>
    <div class="body">
      <div class="form-card">
        <div class="form-card__header">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 2v6M8 10v.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/><circle cx="8" cy="8" r="6.5" stroke="currentColor" stroke-width="1.3"/></svg>
          Formulir Tambah Urgensi
          <span class="req-note"><span class="req">*</span> Wajib diisi</span>
        </div>
        <div class="form-body">
          <div class="field">
            <label class="field__label">Nama Urgensi <span class="req">*</span></label>
            <input v-model="namaUrgensi" type="text" class="field__input" placeholder="Contoh: Rendah, Sedang, Tinggi, Kritis..." @keyup.enter="handleSubmit"/>
          </div>
          <div class="form-actions">
            <button class="fbtn fbtn--ghost" type="button" @click="namaUrgensi=''">Reset</button>
            <button class="fbtn fbtn--primary" type="button" :disabled="isSubmitting" @click="handleSubmit">
              ...
              {{ isSubmitting ? 'Menyimpan...' : 'Simpan Urgensi' }}
            </button>
          </div>
        </div>
      </div>
