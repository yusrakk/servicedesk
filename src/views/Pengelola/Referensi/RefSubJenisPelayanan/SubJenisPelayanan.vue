...
onBeforeMount(() => { fetchData(); fetchLangkahDefault() })

const fetchLangkahDefault = () => {
  const token = localStorage.getItem('Token')
  axios.get('/api/isi_alur', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
      const data = res.data.data
      const awal = data.filter(i => [1,2,3].includes(i.ID_Isi_Alur)).map(i => i.Nama_Alur)
      const akhir = data.find(i => i.ID_Isi_Alur === 4)
      langkahDefault.value = [...awal, akhir?.Nama_Alur || 'Selesai']
    })
    .catch(console.error)
}
...
const fetchData = () => {
  ...
  axios.get('/api/subjenispelayanan', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
      referensi.value = res.data.map(i => ({
        id: i.ID_Sub_Jenis_Pelayanan,
        nama: i.Nama_Sub_Jenis_Pelayanan,
        persyaratan: i.Persyaratan,
        nama_jenis_pelayanan: i.sub__jns_pelayanan.Nama_Jenis_Pelayanan,
        id_jenis_pelayanan: i.sub__jns_pelayanan.ID_Jenis_Pelayanan,
        tglPembuatan: i.created_at || '-'
      }))
    })
    .catch(console.error)
...
const filteredItems  = computed(() => {
  ...
  return referensi.value.filter(i =>
    i.nama.toLowerCase().includes(q) ||
    i.nama_jenis_pelayanan.toLowerCase().includes(q)
  )
})
...
function confirmDelete()  {
  const token = localStorage.getItem('Token')
  axios.delete(`/api/subjenispelayanan/delete/${toDelete.value.id}`, { headers: { Authorization: 'Bearer ' + token } })
    .then(() => { referensi.value = referensi.value.filter(i => i.id !== toDelete.value.id); showModal.value = false })
    .catch(e => alert(e.response?.data?.message || 'Gagal menghapus.'))
}

const baseQuery = (item) => ({
  nama_sub_jenis_pelayanan: item.nama,
  subJenisPelayananId: item.id,
  persyaratan: item.persyaratan,
  JenisPelayananId: item.id_jenis_pelayanan,
  nama_jenis_pelayanan: item.nama_jenis_pelayanan,
  defaultLangkah: JSON.stringify(langkahDefault.value)
})

function editItem(item)  { router.push({ path: '/ubahSubJenisPelayanan', query: baseQuery(item) }) }
function lihatItem(item) { router.push({ path: '/ubahSubJenisPelayanan', query: { ...baseQuery(item), viewOnly: 'true' } }) }
function goTambah()      { router.push({ path: '/tambahSubJenisPelayanan', query: { defaultLangkah: JSON.stringify(langkahDefault.value) } }) }
</script>
  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <h1 class="page-header__title">Referensi Sub Jenis Pelayanan</h1>
        <p class="page-header__sub">Kelola sub jenis pelayanan beserta langkah dan persyaratannya</p>
      </div>
      <button class="btn-tambah" @click="goTambah">
    ...
    <div class="body">
      <div class="toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" placeholder="Cari nama atau jenis pelayanan..." class="search-input"/>
        ...
        <span class="result-count">{{ filteredItems.length }} sub jenis</span>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="filteredItems.length === 0" class="state-center">
          ...
          <p>{{ search ? 'Tidak ada hasil pencarian' : 'Belum ada data sub jenis pelayanan' }}</p>
        </div>
          ...
          <table class="table">
            <thead>
              <tr>
                ...
                <th>Nama Sub Jenis Pelayanan</th>
            ...
            <tbody>
              <tr v-for="(item, i) in paginatedItems" :key="i" class="table-row">
                ...
                <td>
                  <div class="name-cell">
                    <div class="name-icon">
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 10V4l4-2 4 2v6l-4 2-4-2z" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/></svg>
                    </div>
                    <span class="name-text">{{ item.nama }}</span>
                  </div>
                </td>
                <td><span class="jenis-badge">{{ item.nama_jenis_pelayanan }}</span></td>
                <td><span class="date-text">{{ formatDate(item.tglPembuatan) }}</span></td>
                <td>
                  <div class="action-group">
                    <button class="act-btn act-btn--detail" @click="lihatItem(item)" title="Detail">
    ...
    <Teleport to="body">
      <div v-if="showModal" class="modal-overlay" @click.self="cancelDelete">
        <div class="modal">
          ...
          <h3 class="modal__title">Hapus Sub Jenis Pelayanan</h3>
...
.jenis-badge{display:inline-block;padding:.2rem .625rem;border-radius:6px;background:rgba(46,184,106,.1);color:var(--ce);font-size:.75rem;font-weight:600;}
