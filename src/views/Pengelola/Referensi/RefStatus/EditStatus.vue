...
function handleSubmit() {
  if (!namaStatus.value.trim()) { alert('Nama status wajib diisi'); return }
  isSubmitting.value = true
  const token = localStorage.getItem('Token')
  axios.put(`/api/status/${statusId.value}`, { Nama_Status: namaStatus.value }, { headers: { Authorization: 'Bearer ' + token } })
    .then(() => router.push('/referensi/status'))
...
<template>
  <div class="form-page">
    <div class="page-header">
      <button class="back-btn" @click="router.push('/referensi/status')">
      ...
      <div>
        <span class="page-header__label">Referensi · Status</span>
        <h1 class="page-header__title">Ubah Status</h1>
        <p class="page-header__sub">Edit nama status: <strong style="color:white">{{ route.query.nama_status }}</strong></p>
      </div>
      <div class="orb"></div>
    </div>
    <div class="body">
      <div class="form-card">
        <div class="form-card__header">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M12 2l2 2-9 9H3V11l9-9z" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
          Formulir Ubah Status
          <span class="req-note"><span class="req">*</span> Wajib diisi</span>
        </div>
        <div class="form-body">
          <div class="info-note">
            <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5.5" stroke="currentColor" stroke-width="1.2"/><path d="M6.5 4.5v3M6.5 9h.01" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>
            Perubahan nama status akan berpengaruh pada seluruh data pelayanan yang menggunakan status ini.
          </div>
          <div class="field">
            <label class="field__label">Nama Status <span class="req">*</span></label>
            <input v-model="namaStatus" type="text" class="field__input" placeholder="Nama status..." @keyup.enter="handleSubmit"/>
          </div>
          <div class="form-actions">
            <button class="fbtn fbtn--ghost" type="button" @click="router.push('/referensi/status')">Batal</button>
...
.info-note{display:flex;align-items:flex-start;gap:.5rem;font-size:.8rem;color:#92400e;background:#fffbeb;border:1px solid #fcd34d;border-radius:9px;padding:.625rem .875rem;margin-bottom:1.25rem;line-height:1.5;}
