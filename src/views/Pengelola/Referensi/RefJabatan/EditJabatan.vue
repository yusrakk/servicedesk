...
function handleSubmit() {
  if (!namaJabatan.value.trim()) { alert('Nama jabatan wajib diisi'); return }
  isSubmitting.value = true
  const token = localStorage.getItem('Token')
  axios.put(`/api/jabatan/${jabatanId.value}`, { Nama_Jabatan: namaJabatan.value }, { headers: { Authorization: 'Bearer ' + token } })
    .then(() => router.push('/referensi/jabatan'))
    .catch(e => alert(e.response?.data?.message || 'Gagal mengubah jabatan.'))
...
<template>
  <div class="form-page">
    <div class="page-header">
      <button class="back-btn" @click="router.push('/referensi/jabatan')">
      ...
      <div>
        <span class="page-header__label">Referensi · Jabatan</span>
        <h1 class="page-header__title">Ubah Jabatan</h1>
        <p class="page-header__sub">Edit data jabatan: <strong style="color:white">{{ route.query.nama_jabatan }}</strong></p>
      </div>
      <div class="orb"></div>
    </div>
    <div class="body">
      <div class="form-card">
        ...
        <div class="form-body">
          <div class="field">
            <label class="field__label">Nama Jabatan <span class="req">*</span></label>
            <input v-model="namaJabatan" type="text" class="field__input" placeholder="Nama jabatan..." @keyup.enter="handleSubmit"/>
          </div>
          <div class="form-actions">
            <button class="fbtn fbtn--ghost" type="button" @click="router.push('/referensi/jabatan')">Batal</button>
