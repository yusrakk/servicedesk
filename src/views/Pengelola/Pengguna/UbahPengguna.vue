...
Promise.all([
  ...
  const matchRole = pilihanRole.value.find(r => r.label === role.value)
  if (matchRole) idRoleTerpilih.value = matchRole.id

  const matchJab = pilihanJabatan.value.find(j => j.label === route.query.jabatan)
  if (matchJab) idJabatanTerpilih.value = matchJab.id

  const matchOrg = pilihanInduk.value.find(o => o.label === organisasi.value)
  if (matchOrg) idOrganisasiTerpilih.value = matchOrg.id
}).catch(console.error)
...
function handleSubmit() {
  ...
  axios.put(`/api/user/${idUser.value}`, {
    Nama_Depan: Nama_Depan.value, Nama_Belakang: Nama_Belakang.value,
    ID_Role: idRoleTerpilih.value, ID_Jabatan: idJabatanTerpilih.value,
    ID_Organisasi: idOrganisasiTerpilih.value, Password: password.value
  }, { headers: hdr })
  .then(() => router.push('/pengguna'))
  .catch(e => alert(e.response?.data?.message || 'Gagal mengubah pengguna.'))
  ...
  <div class="form-page">
    <div class="page-header">
      ...
      <div>
        ...
        <h1 class="page-header__title">Ubah Pengguna</h1>
        <p class="page-header__sub">Edit data akun pengguna: <strong style="color:white">{{ Nama_Depan }} {{ Nama_Belakang }}</strong></p>
      </div>
    ...
    <div class="body">
      <div class="form-card">
        <div class="form-card__header">
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M13 2l3 3-10 10H3V12L13 2z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
        ...
        <div class="form-body">
          <div class="form-row">
            <div class="field">
              ...
              <input v-model="Nama_Depan" type="text" class="field__input" placeholder="Nama depan"/>
            </div>
            <div class="field">
              <label class="field__label">Nama Belakang <span class="req">*</span></label>
              <input v-model="Nama_Belakang" type="text" class="field__input" placeholder="Nama belakang"/>
            </div>
          </div>
          <div class="form-row">
            <div class="field">
          ...
          <div class="field">
            <label class="field__label">Perangkat Daerah / Organisasi</label>
          ...
          <div class="form-row">
            <div class="field">
              <label class="field__label">Password Baru <span class="req">*</span></label>
              <div class="pass-wrap">
          ...
          <div class="form-actions">
            <button type="button" class="fbtn fbtn--ghost" @click="router.push('/pengguna')">Batal</button>
...
@media(max-width:640px){.form-row{grid-template-columns:1fr;}.page-header{flex-direction:column;padding:1.25rem;}.body{padding:1rem;}}
</style>