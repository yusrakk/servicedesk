...
// Hasil Pemenuhan
const pesanHasilPemenuhan = ref('')
const fileHasilPemenuhan  = ref(null)
const fileHasilNama       = ref('')
...
const fetchData = async () => {
  if (dataCache.value?.id === pelayananId.value && !forceRefresh.value) {
    const c = dataCache.value
    Object.keys(c).forEach(k => { if (k !== 'id') eval(`${k}.value = c['${k}']`) })
  ...
  try {
    ...
    sub_jenis_pelayanan.value = d.sub_jenis_pelayanan?.Nama_Sub_Jenis_Pelayanan || ''
    jenis_pelayanan.value  = d.jenis_pelayanan?.Nama_Jenis_Pelayanan || ''
...
async function addMessage() {
  ...
  fd.append('ID_User', userId.value)
  fd.append('Pesan', newMessage.value)
...
// File hasil
function handleFileHasil(e) {
  const f = e.target.files[0]; if (!f) return
  if (f.type !== 'application/pdf') { alert('Hanya PDF.'); e.target.value=''; return }
  if (f.size > 8*1024*1024) { alert('Maks 8MB.'); e.target.value=''; return }
  fileHasilPemenuhan.value = f; fileHasilNama.value = f.name
}

async function submitHasilPemenuhan() {
  isSubmitting.value = true
  const token = localStorage.getItem('Token')
  try {
    const fd = new FormData()
    if (fileHasilPemenuhan.value) fd.append('hasil_pemenuhan', fileHasilPemenuhan.value)
    await axios.put(`/api/pelayanan/tambah/hasil/${pelayananId.value}`, fd, {
      headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'multipart/form-data' }
    })
    if (pesanHasilPemenuhan.value.trim()) {
      const md = new FormData()
      md.append('ID_User', userId.value); md.append('Pesan', pesanHasilPemenuhan.value)
      await axios.post(`/api/pesan/${pelayananId.value}`, md, { headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'multipart/form-data' } })
    }
    dataCache.value = null; forceRefresh.value = true
    await fetchData(); forceRefresh.value = false
    pesanHasilPemenuhan.value = ''; fileHasilPemenuhan.value = null; fileHasilNama.value = ''
    showKonfirmasi.value = false
    return true
  } catch (e) { console.error(e); alert('Gagal mengirim hasil pemenuhan.'); return false }
...
onMounted(() => {
  if (!pelayananId.value || pelayananId.value === '-') { router.push('/permintaanTeknis'); return }
  fetchData()
  const onPop = () => router.push('/permintaanTeknis')
  ...
  <div class="page">
    ...
    <template v-else-if="isDataLoaded">
      ...
      <div class="page-header">
        <button class="back-btn" @click="router.push('/permintaanTeknis')">
        ...
        <div style="flex:1">
          <span class="page-header__label">Pelaksana Teknis · Detail</span>
          <h1 class="page-header__title">{{ perihal || 'Detail Permintaan' }}</h1>
      ...
      <div class="body">
        ...
        <div v-if="activeTab==='informasi'" class="two-col">
          ...
          <div class="info-col">
            <div class="section-card">
              ...
              <div class="section-card__body">
                <div class="info-grid">
                  ...
                  <div class="ig-item"><span class="ig-label">Layanan</span><span class="ig-value">{{ sub_jenis_pelayanan }}</span></div>
                  <div class="ig-item"><span class="ig-label">Tipe</span><span class="ig-value">{{ jenis_pelayanan }}</span></div>
                  <div class="ig-item ig-item--full"><span class="ig-label">Perangkat Daerah</span><span class="ig-value">{{ organisasi }}</span></div>
            ...
            <!-- Hasil Pemenuhan (jika sudah selesai) -->
            <div v-if="status === 5 || status === 6" class="section-card">
              <div class="section-card__header">
              ...
              <div class="section-card__body">
                <div class="doc-item" v-for="(doc, i) in [
                  ...
                  <span v-if="doc.path && doc.path!=='-'" class="doc-dot doc-dot--ada"></span>
                  <span v-else class="doc-dot doc-dot--kosong"></span>
                </div>
              </div>
            </div>
          </div>

          <!-- Kanan: Chat + Submit Hasil -->
          <div class="chat-col">
            ...
            <!-- Submit Hasil Pemenuhan -->
            <div v-if="status !== 5 && status !== 6" class="section-card">
              <div class="section-card__header">
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 7l3 3 6-6" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                Kirim Hasil Pemenuhan
              </div>
              <div class="section-card__body">
                <div class="field">
                  <label class="flabel">Pesan Hasil Pemenuhan</label>
                  <textarea v-model="pesanHasilPemenuhan" class="ftextarea" placeholder="Tulis pesan hasil pemenuhan..." rows="3"></textarea>
                </div>
                <div class="field">
                  <label class="flabel">File Hasil Pemenuhan <span style="font-size:.68rem;color:#6b7280;text-transform:none;letter-spacing:0">(PDF, maks. 8MB)</span></label>
                  <label class="file-upload-btn">
                    ...
                    {{ fileHasilNama || 'Pilih File PDF' }}
                    <input type="file" accept=".pdf" style="display:none" @change="handleFileHasil"/>
                  </label>
                </div>
    ...
    <Teleport to="body">
      <div v-if="showKonfirmasi" class="modal-overlay" @click.self="showKonfirmasi=false">
        <div class="modal">
          ...
          <h3 class="modal__title">Konfirmasi Kirim</h3>
          <p class="modal__desc">Pastikan data hasil pemenuhan sudah benar sebelum mengirim.</p>
          <div class="modal__actions">
            <button class="fbtn fbtn--primary" :disabled="isSubmitting" @click="submitHasilPemenuhan">
...
.tabs-bar{display:flex;gap:0;background:white;border-bottom:1px solid rgba(168,200,180,.2);padding:0 1.5rem;}
...
.ftextarea:focus{border-color:var(--cm);}
...
.chat-messages{padding:1rem;min-height:200px;max-height:320px;overflow-y:auto;display:flex;flex-direction:column;gap:.625rem;background:var(--cmi);}
...
.step-row{display:flex;align-items:flex-start;gap:.875rem;position:relative;}
.step-circle{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:700;flex-shrink:0;z-index:1;}
...
.step-line{position:absolute;left:13px;top:28px;width:2px;height:24px;z-index:0;}
