...
onMounted(() => {
  ...
  axios.get('/api/pelayanan/hasil/teknis', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
      layananData.value = res.data.map(i => ({
        noTiket:  i.ID_Pelayanan,
        perihal:  i.Perihal,
        pelapor:  i.Nama_Pelapor,
        tanggal:  i.created_at,
        status:   i.status_pelayanan?.Nama_Status || '-',
        terkirim: i.Is_Done,
        urgensi:  i.urgensi_pelayanan?.Nama_Urgensi || '-',
      }))
    })
    .catch(console.error)
...
function lihatDetail(item) {
  router.push({ name: 'DetailHasilPengerjaan', query: { layanan: item.noTiket } })
}
...
const filteredItems = computed(() => {
  ...
  let items = layananData.value.filter(i =>
    ...
    i.pelapor?.toLowerCase().includes(q) ||
    i.status?.toLowerCase().includes(q) ||
    i.urgensi?.toLowerCase().includes(q) ||
  ...
  <div class="page">
    <div class="page-header">
      <div>
        ...
        <h1 class="page-header__title">Hasil Pengerjaan</h1>
        <p class="page-header__sub">Riwayat pelayanan yang telah dikerjakan</p>
      </div>
    ...
    <div class="body">
      <div class="toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" class="search-input" placeholder="Cari perihal, pelapor, status..."/>
        ...
        <span class="result-count">{{ filteredItems.length }} hasil</span>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="filteredItems.length === 0" class="state-center">
          ...
          <p>{{ search ? 'Tidak ada hasil pencarian' : 'Belum ada hasil pengerjaan' }}</p>
        </div>
            ...
            <thead>
              <tr>
                ...
                <th style="width:140px;cursor:pointer" @click="toggleSort('status')">
                  Status <span class="sort-icon">{{ sortKey==='status'?(sortOrder==='asc'?'↑':'↓'):'↕' }}</span>
                </th>
              ...
              <tr v-for="(item, i) in paginatedItems" :key="i" class="table-row">
                ...
                <td>
                  <div class="perihal-cell">
                    ...
                    <span class="pelapor-text">{{ item.pelapor }}</span>
                  </div>
                </td>
                <td><span class="date-text">{{ formatDate(item.tanggal) }}</span></td>
                <td>
                  <span class="badge" :style="{ color: urgensiConfig[item.urgensi]?.color||'#374151', background: urgensiConfig[item.urgensi]?.bg||'#f3f4f6' }">
                    <span class="bdot" :style="{ background: urgensiConfig[item.urgensi]?.color||'#374151' }"></span>
                    {{ item.urgensi }}
                  </span>
                </td>
                <td>
                  <span class="badge" :style="{ color: statusConfig[item.status]?.color||'#374151', background: statusConfig[item.status]?.bg||'#f3f4f6' }">
                ...
                <td>
                  <div class="action-cell">
                    ...
                    <span class="terkirim-dot" :class="item.terkirim ? 'terkirim-dot--ada' : 'terkirim-dot--kosong'" :title="item.terkirim ? 'Hasil terkirim' : 'Belum terkirim'"></span>
                  </div>
                </td>
              </tr>
            </tbody>
      ...
      <div class="legend">
        <span class="terkirim-dot terkirim-dot--ada"></span> Hasil sudah terkirim
        <span class="terkirim-dot terkirim-dot--kosong" style="margin-left:.875rem"></span> Belum terkirim
      </div>
    </div>
  </div>
</template>
...
.body{padding:1.5rem;max-width:1100px;margin:0 auto;display:flex;flex-direction:column;gap:.875rem;}
...
.pelapor-text{font-size:.72rem;color:var(--csv);}
.date-text{font-size:.8rem;color:var(--cs);}
.badge{display:inline-flex;align-items:center;gap:.35rem;padding:.25rem .7rem;border-radius:99px;font-size:.75rem;font-weight:700;}
.bdot{width:5px;height:5px;border-radius:50%;flex-shrink:0;}
.action-cell{display:flex;align-items:center;gap:.625rem;}
...
.terkirim-dot{width:9px;height:9px;border-radius:50%;flex-shrink:0;display:inline-block;}
.terkirim-dot--ada{background:#16a34a;}
.terkirim-dot--kosong{background:#d1d5db;}
.legend{display:flex;align-items:center;gap:.375rem;font-size:.75rem;color:var(--cs);}
