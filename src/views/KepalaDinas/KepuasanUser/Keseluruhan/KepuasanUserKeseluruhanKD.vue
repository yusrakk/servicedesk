  ...
  axios.get('/api/pelayanan', { headers: { Authorization: 'Bearer ' + token } })
    .then(res => {
      items.value = res.data
        .filter(i => ['Selesai', 'Tutup', 'Ditolak'].includes(i.status_pelayanan?.Nama_Status))
        .map(i => ({
          ticket: i.ID_Pelayanan,
          perihal: i.Perihal,
          pic:    i.teknis_pelayanan?.Nama_Depan || '-',
          date:   i.created_at,
...
function checkProgress(item) {
  router.push({ name: 'DetailKepuasanUserKeseluruhan', query: { layanan: item.ticket } })
}
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(i =>
    ...
    i.ticket?.toString().toLowerCase().includes(q) ||
...
<template>
  <div class="page">
    <div class="page-header">
      <div>
        <span class="page-header__label">Kepuasan User</span>
        <h1 class="page-header__title">Laporan Kepuasan Keseluruhan</h1>
        <p class="page-header__sub">Seluruh pelayanan berstatus Selesai, Tutup, atau Ditolak</p>
      </div>
      <div class="orb"></div>
    </div>
    ...
    <div class="body">
      <div class="toolbar">
        <div class="search-box">
          ...
          <input v-model="search" type="text" class="search-input" placeholder="Cari tiket, perihal, PIC, status..."/>
        ...
        <span class="result-count">{{ filteredItems.length }} laporan</span>
      </div>
      ...
      <div class="table-card">
        ...
        <div v-else-if="filteredItems.length === 0" class="state-center">
          ...
          <p>{{ search ? 'Tidak ada hasil pencarian' : 'Tidak ada data kepuasan' }}</p>
        </div>
        <div v-else class="table-wrap">
          <table class="table">
            ...
            <tbody>
              <tr v-for="item in paginatedItems" :key="item.ticket" class="table-row">
                <td><span class="ticket-chip">#{{ item.ticket }}</span></td>
                <td><span class="perihal-text">{{ item.perihal }}</span></td>
                <td><span class="date-text">{{ formatDate(item.date) }}</span></td>
                <td><span class="pic-text">{{ item.pic }}</span></td>
                <td>
                  <span class="badge"
                ...
                <td>
                  <button class="act-btn" @click="checkProgress(item)">
...
.orb{position:absolute;width:280px;height:280px;background:var(--cm);border-radius:50%;filter:blur(80px);opacity:.1;top:-80px;right:-40px;pointer-events:none;}
...
.pic-text{font-size:.8rem;color:var(--cs);}
