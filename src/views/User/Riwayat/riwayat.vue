...
onMounted(() => {
  ...
  .then(response => {
    items.value = response.data.filter(item =>
        ['Tutup', 'Ditolak', 'Selesai'].includes(item.status_pelayanan?.Nama_Status)
      ).map(item => ({
...
function checkProgress(item) {
  console.log('Navigating to detail with ticket:', item.ticket)
  router.push({
      ...
      <table class="rounded-table">
        ...
        <tbody>
          ...
          <tr v-for="item in paginatedItems" :key="item.ticket">
            ...
            <td>
              <a href="#" @click.prevent="checkProgress(item)" style="color: blue; text-decoration: underline;">Lihat Detail</a>
            </td>
          </tr>
        </tbody>
      </table>
